...
const DisplayActiveStep = () => {
  ...
  useEffect(() => {
    console.log("Inside VerificationSection Index");
    console.log("QRdata: ", activeStep);
    if (qrData === "") return;
    try {
      setActiveStep(VerificationSteps.Verifying);
      decodeQrData(qrData).then((data) => {
        console.log("Parse issue debug log :", qrData);
  ...
  switch (activeStep) {
    ...
    case VerificationSteps.DisplayResult:
      return (
        <Result setActiveStep={setActiveStep} vc={vc} vcStatus={vcStatus} />
      );
    case VerificationSteps.ActivateCameraPopUp:
      return <></>;
    default:
      return <></>;
  }
